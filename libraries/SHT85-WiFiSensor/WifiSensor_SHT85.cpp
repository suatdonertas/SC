...
WifiSensor_SHT85::WifiSensor_SHT85() {
  _i2caddr = NULL;
  humidity = 0.0f;
  temp = 0.0f;
}

boolean WifiSensor_SHT85::begin(uint8_t i2caddr,int sda_pin,int scl_pin) {
...
uint16_t WifiSensor_SHT85::readStatus(void) {
...
void WifiSensor_SHT85::reset(void) {
  writeCommand(SHT85_SOFTRESET);
  delay(10);
}

void WifiSensor_SHT85::heater(boolean h) {
  if (h)
...
float WifiSensor_SHT85::readTemperature(void) {
  if (!readTempHum())
    return NAN;
...
float WifiSensor_SHT85::readHumidity(void) {
  if (!readTempHum())
    return NAN;
...
boolean WifiSensor_SHT85::readTempHum(void) {
...
void WifiSensor_SHT85::writeCommand(uint16_t cmd) {
...
uint8_t WifiSensor_SHT85::crc8(const uint8_t *data, int len) {
  /*
   *
